...
import ssm.catalog.models.Dataset;
...
import ssm.catalog.utils.RdfDatasetWriter;
...
public class DatasetController {
    ...
    */
    @Autowired
    private DatasetUtils modelUtils;
    ...
    */
     ...
     *
     * @param collectionTitle    Title of the collection for the model
    ...
    */
    ...
    private Map<String, Object> constructModelsBody(
        ...
        final int modelCount = DatasetSparql.getModelCount(endpointUrl);
        /*
     ...
     *
     * @param collectionTitle  Dataset title
     * @param modelUUID     Model UUID
     * @param model         Model to upload
     * @return BatsModel of the new upload Model
     * @throws IOException
     */
    private Dataset uploadToGraphDatabase(
        final String collectionTitle,
        final String modelUUID,
        final Model  model
    ) throws IOException, ResponseStatusException {
        CustomizedCollection collection = collectionUtils.getDataset(collectionTitle);
        ...
        return new Dataset(modelUUID, RdfModelWriter.getJsonldForModel(newModel));
    }
    ...
    private void uploadToDocumentStore(
        ...
        + fuseki().getPort()
        + "/" + collectionTitle;
        Model model = modelUtils.getModel(collectionTitle, modelUUID);
        String modelUri = configUtils.getModelUri(collectionTitle, modelUUID);
    ...
    /**
     * FETCH a certain amount of collections.
     *
     * @param collectionTitle Title of the Apache Jena Dataset this model belongs to
    ...
    public ResponseEntity<?> queryModels(
        @PathVariable("collection_title") @Pattern(regexp = Collection.TITLE_REGEX)
        final String collectionTitle,
        ...
        String modelsUri = configUtils.getDatasetUri(collectionTitle) + "/models";
        //Add each found model to the response
        try {
            if (returnFull) {
                List<Dataset> models = DatasetSparql.getFullModels(
                    pageSize,
                    pageNumber,
                    endpointUrl,
                    collection
                ...
                List<Map<String, Object>> models = DatasetSparql.getModelSummaries(
                    pageSize,
                    pageNumber,
                    endpointUrl
                );
     ...
     *
     * @param collectionTitle Title for Dataset collection to add the new Model
    ...
    */
    ...
    public Dataset createModel(
        ...
        String jsonld = transformJsonld(collectionTitle, modelUUID, jsonldPayload);
        ...
        try {
            ...
            batsModel = uploadToGraphDatabase(collectionTitle, modelUUID, model);
        } catch (Exception e) {
            LOGGER.error(UPLOAD_MODEL_ERROR, e);
            throw new ResponseStatusException(
        ...
        try {
            uploadToDocumentStore(collectionTitle, modelUUID, jsonld);
     ...
     *
     * @param collectionTitle Title for Dataset collection that Model belonds to
    ...
    */
    ...
    public ResponseEntity<?> getModel(
        ...
        final DatasetFormats format
    ) throws IOException, ResponseStatusException {
        // Return full, graph JSON-LD of model
        if (format == DatasetFormats.GRAPH || format == DatasetFormats.FULL) {
            Model model = modelUtils.getModel(collectionTitle, modelUUID);
            Dataset batsModel = new Dataset(
                modelUUID,
        ...
        if (format == DatasetFormats.JSONLD) {
    ...
    public ResponseEntity<?> getUUIDs(@PathVariable("collection_title")
        @Pattern(regexp = Collection.TITLE_REGEX) final String collectionTitle
    ) {
        // Check if collection exists
        collectionUtils.getDataset(collectionTitle);
        ...
        ArrayNode uuidArray;
        try {
            uuidArray = DatasetSparql.getModelUuids(endpointUrl);
        } catch (QueryException ex) {
    ...
    */
    ...
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    public ResponseEntity<Dataset> updateModelReplace(
        @PathVariable("collection_title") @Pattern(regexp = Collection.TITLE_REGEX)
        final String collectionTitle,
        @PathVariable("model_uuid") @Pattern(regexp = UUIDGenerator.UUID_REGEX)
        final String modelUUID,
        ...
        /*
        Get the collection's model. We want to extract the created timestamp,
        ...
        */
        ...
        String jsonldForGraph = transformJsonld(collectionTitle, modelUUID, jsonldPayload);
        Model model = jsonldToModel(jsonldForGraph, modelUUID, createdTimeNode.textValue());
        Dataset batsModel = uploadToGraphDatabase(collectionTitle, modelUUID, model);

        // Add updated model to document store
        LOGGER.info("Uploading model to document store: " + modelUUID);
        String jsonldForDocument = transformJsonld(collectionTitle, modelUUID, jsonldPayload);
        try {
            uploadToDocumentStore(collectionTitle, modelUUID, jsonldForDocument);
            LOGGER.info("Model uploaded to document store!");
        } catch (Exception e) {
            ...
            uploadToGraphDatabase(collectionTitle, modelUUID, oldModel);
            LOGGER.error(UPLOAD_MODEL_ERROR, e);
            throw new ResponseStatusException(
                HttpStatus.INTERNAL_SERVER_ERROR,
                "Model unable to be uploaded to document store"
    ...
    */
    ...
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    public ResponseEntity<Dataset> updateModelPartial(
        @PathVariable("collection_title") @Pattern(regexp = Collection.TITLE_REGEX)
        final String collectionTitle,
        @PathVariable("model_uuid") @Pattern(regexp = UUIDGenerator.UUID_REGEX)
        final String modelUUID,
        ...
        // Add updated model to graph database
        Dataset mergedBatsModel;
        try {
            LOGGER.info("Uploading model to graph database: " + modelUUID);
            Model model = jsonldToModel(mergedModelJsonld, modelUUID, createdTimeNode.textValue());
            mergedBatsModel = uploadToGraphDatabase(collectionTitle, modelUUID, model);
        } catch (Exception e) {
            LOGGER.error(UPLOAD_MODEL_ERROR, e);
            throw new ResponseStatusException(
                HttpStatus.INTERNAL_SERVER_ERROR,
        ...
        // Add updated model to document store
        try {
            uploadToDocumentStore(collectionTitle, modelUUID, mergedModelJsonld);
            LOGGER.info("Model uploaded to document store!");
        } catch (Exception e) {
            // Rollback graph database to original model
            LOGGER.error("Unable to update model in document store: " + modelUUID);
            ...
            uploadToGraphDatabase(collectionTitle, modelUUID, model);
            LOGGER.error(UPLOAD_MODEL_ERROR, e);
            throw new ResponseStatusException(
                HttpStatus.INTERNAL_SERVER_ERROR,
                "Model unable to be uploaded to document store"
